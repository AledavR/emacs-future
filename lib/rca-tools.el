...
(use-package denote
  :ensure t
  :config
  (setq denote-directory (concat (xdg-user-dir "DOCUMENTS") "/notes")))
;; Denote:1 ends here
...
(use-package citar
  ...
  (let ((documents-path (xdg-user-dir "DOCUMENTS")))
    (setq citar-file-note-extensions '("org")
          citar-notes-paths `( ,(concat documents-path "/articles/notes/"))
          citar-library-paths `( ,(concat documents-path "/articles/"))
          citar-library-file-extensions '("pdf")
          citar-bibliography `(,(concat documents-path "/bibliography.bib")))))
